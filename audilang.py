...
pixels = Neopixel(num_leds=170, state_machine=0, pin=14, mode="GRB")
...
def audi_blinker():
    farben = (orange, blue, red, green, indigo, violet, yellow)
    farbindex = random.randint(0, 6)
    farbindex2 = farbindex+1
    if farbindex2 > 6:
        farbindex2 = 0
    pixels.clear()
    pixels.show()
    time.sleep(0.4)
    for pos in range(0, 85):
        pixels.set_pixel(pos, farben[farbindex])
        pixels.set_pixel(169-pos, farben[farbindex2])
        pixels.show()
        time.sleep(0.02)
    time.sleep(0.2)
    pixels.clear()
    ...
    for i in range(0, 169):
       pixels.set_pixel(i, farben[farbindex])
    pixels.show()
    time.sleep(0.3)
    for i in range(0, 169):
       pixels.set_pixel(i, farben[farbindex2])
    pixels.show()
    time.sleep(0.3)
    for i in range(0, 169):
       pixels.set_pixel(i, farben[farbindex])
...
#rotate_right(10, 12, orange, orange)  
